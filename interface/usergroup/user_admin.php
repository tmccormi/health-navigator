...
?>
  ...
  <tr>
  	<td><span class=text><?php xl('Provider Exams','e'); ?>:</span></td>
	<td>
		<?php 
		require_once( __DIR__.'/../../mi2Lib/php/Mi2/Api.php' );
		$api = Mi2_Api::getInstance();
		$userExamTypes = $api->fetchExamTypesForUser( $_GET['id'] );
		?>
		<select name="provider_exam_types[]" multiple width="200" style="width: 200px">
		<?php foreach ( $api->fetchAllExamTypes() as $examType ) { ?> 
		 	<option value="<?php echo $examType->pc_catid ?>" <?php if ( in_array( $examType->pc_catid, $userExamTypes ) ) echo "selected" ?>><?php echo $examType->pc_catname ?></option>
		<?php } ?> 
		</select>
	</td>
  </tr>
  ...
  <tr>
  	<td><span class=text><?php xl('Slots Pledged','e'); ?>:</span></td>
	<td>
		<input type="text" name="provider_slots_pledged" value="<?php echo $iter['provider_slots_pledged'] ?>" />
	</td>
  </tr>
  <tr>
  	<td><span class=text><?php xl('Slots Used','e'); ?>:</span></td>
	<td>
		<input type="text" name="provider_slots_used" value="<?php echo $iter['provider_slots_used'] ?>" />
	</td>
  </tr>
  <tr>
  	<td><span class=text><?php xl('Slots Remaining','e'); ?>:</span></td>
	<td>
	   <?php $remaining = max( 0, $iter['provider_slots_pledged'] - $iter['provider_slots_used'] ) ?>
