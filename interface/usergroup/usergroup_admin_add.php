...
?>
  ...
  <tr>
	...
	<td>
		...
		?>
		<input type="hidden" name="user-id" value="<?php echo $_SESSION['authUserID'] ?>" />
		<select name="provider_exam_types[]" multiple style="width=200px;">
		<?php foreach ( $api->fetchAllExamTypes() as $examType ) { ?> 
		 	<option value="<?php echo $examType->pc_catid ?>"><?php echo $examType->pc_catname ?></option>
		<?php } ?> 
		</select>
	</td>
  </tr>
	...
	<td>
		<input type="text" name="provider_slots_pledged" />
	</td>
  </tr>
  <tr>
  	<td><span class=text><?php xl('Slots Used','e'); ?>:</span></td>
	<td>
		<input type="text" name="provider_slots_used" />
	</td>
  </tr>
