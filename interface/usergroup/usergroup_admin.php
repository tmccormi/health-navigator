...
if ($_GET["privatemode"]=="user_admin") {
    if ($_GET["mode"] == "update") {
      ...
      // KCC 
      if ( $_GET['provider_exam_types'] ) {
          require_once( __DIR__.'/../../mi2Lib/php/Mi2/Api.php' );
          $api = Mi2_Api::getInstance();
          $api->setExamsForProvider( $_GET['provider_exam_types'], $_GET['id'] );
      }
      
      if ( $_GET["provider_slots_pledged"] ) {
          $tqvar = formData('provider_slots_pledged','G');
          sqlStatement("update users set provider_slots_pledged='$tqvar' where id={$_GET["id"]}");
      }
      
      if ( $_GET["provider_slots_used"] ) {
        $tqvar = formData('provider_slots_used','G');
        sqlStatement("update users set provider_slots_used='$tqvar' where id={$_GET["id"]}");
      }
...
if (isset($_POST["mode"])) {
  if ($_POST["mode"] == "new_user") {
	...
	"', newcrop_user_role = '"  . trim(formData('erxrole' )) .
      ...
      // KCC
      if ( $_POST['provider_exam_types'] ) {
          require_once( __DIR__.'/../../mi2Lib/php/Mi2/Api.php' );
          $api = Mi2_Api::getInstance();
          $api->setExamsForProvider( $_POST['provider_exam_types'], $prov_id );
      }
      
    if ( $_POST['provider_slots_pledged'] ) {
          require_once( __DIR__.'/../../mi2Lib/php/Mi2/Api.php' );
          $api = Mi2_Api::getInstance();
          $api->setProviderSlotsRemaining( $_POST['provider_slots_pledged'], $prov_id );
      }
      
      if ( $_POST['provider_slots_used'] ) {
          require_once( __DIR__.'/../../mi2Lib/php/Mi2/Api.php' );
          $api = Mi2_Api::getInstance();
          $api->setProviderSlotsRemaining( $_POST['provider_slots_used'], $prov_id );
      }
