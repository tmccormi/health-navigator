...
$(document).on( 'change', '#ActionsInProgress', function( e ) {
    var val = $(this).val();	
    var controller = $('option:selected', this).attr('data-controller');
    var action = $('option:selected', this).attr('data-action');
    top.window.parent.left_nav.loadFrame( 'enc','RTop', 'navigator/index.php?action='+controller+'!'+action+'&'+controller+'Id='+val)
	return true;
});
...
//It opens the corresponding pages.
	...
	if(rawdata=='')
	 ...
	 {
	 	top.window.parent.left_nav.loadFrame2('pel1','RBot','navigator/index.php?action=case!current')
		return true;
	 }
...
<td style="margin:3px 0px 3px 0px;vertical-align:middle;" align="left">
	<div id="gtc-active-action-container"></div>
</td>
