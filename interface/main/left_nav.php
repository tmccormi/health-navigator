<?php
 ...
 $primary_docs = array(
  ...
  'ens' => array(xl('Visit History'), 1, 'navigator/index.php?action=case!current'),
  ...
  'edi' => array(xl('EDI History') , 0, 'billing/edih_view.php'),
    'cnc' => array(xl('New Case') , 1, 'navigator/index.php?action=case!create'),
    'ccc' => array(xl('Current') , 1, 'navigator/index.php?action=case!current'),
    'cch' => array(xl('Case History') , 0, 'navigator/index.php?action=mycases!list')
 );
...
function getEncounterTargetFrame( name ) {
 ...
 function setEncounterNoShow( edate, eid, frname ) 
 {
	 if (eid == active_encounter) return;
	  active_encounter = eid;
	  reloadEncounter(frname);
...
$(document).ready(function(){
  if(3 == <?php echo $GLOBALS['concurrent_layout'] ?>){
    ...
    $("#navigation-slide > li  > a#gtc0").prepend('<img src="../../images/file.png" class="nav-menu-img" />');
...
<?php } ?>
    ...
    <?php genMiscLink( 'RTop', 'gtc', '0', xl( 'My Cases' ), 'navigator/index.php?action=mycases!list', true ); ?> 

    <?php genMiscLink( 'RTop', 'gtc', '0', xl( 'Providers' ), 'navigator/index.php?action=providers!list', true ); ?> 
...
<?php } else { // not athletic team ?>
  ...
  <li class="open"><a class="expanded" id="patimg" ><span><?php xl('Patient/Client','e') ?></span></a>
    <ul>
      ...
      <li class="open"><a class="expanded_lv2"><span><?php xl('Cases','e') ?></span></a>
        <ul>
          <?php genTreeLink('RBot','cnc',xl('New Case')); ?>
          <?php genTreeLink('RBot','ccc',xl('Current')); ?>
          <?php genTreeLink('RBot','cch',xl('Case History')); ?>
        </ul>
      </li>
