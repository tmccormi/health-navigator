...
function myGetRegistered($state="1", $limit="unlimited", $offset="0") {
  ...
  if ($res) {
    for($iter=0; $row=sqlFetchArray($res); $iter++) {
        $actionTypeJson = $row['category'];
        $actionTypes = json_decode( $actionTypeJson );
        $currentActionTypeId = Gtc_Helper::getActiveAction()->getActionType()->getActionTypeId();
        if ( in_array( $currentActionTypeId, $actionTypes ) ) {
            $row['category'] = xl('Forms');
            $all[$iter] = $row;
        }
    }
  }
