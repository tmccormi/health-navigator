...
<?php
	foreach($_POST as $key=>$val) {
	       ...
	       if (preg_match('/category_(\d+)/', $key, $matches)) {
			...
			sqlQuery("update registry set category='".$json."' where id=".$category_id);
		}
...
foreach($bigdata as $registry)
{
	...
	<tr>
		...
		<?php
		    // include the Gtc_Helper
		    require_once( __DIR__."/../navigator/library/Gtc/Helper.php" );
			echo "<td><input type=text size=4 name=priority_".$registry['id']." value='".$priority_category['priority']."'></td>";
			$json = $priority_category['category'];
			$category_array = json_decode( $json );
			echo "<td><select multiple size=8 name=category_".$registry['id']."[]>";
			foreach ( Gtc_Helper::getActionTypes() as $actionType ) {
			    echo "<option ";
			    echo in_array( $actionType->getActionTypeId(), $category_array ) ? "selected" : "";
			    echo " value='".$actionType->getActionTypeId()."'>".$actionType->getName()."</option>";
			}
			echo "</select></td>";
